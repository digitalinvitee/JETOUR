/* Safe GSAP fallback: keeps the page usable even if the CDN is blocked or slow. */
(function () {
  function applyVars(target, vars) {
    const items = typeof target === "string" ? document.querySelectorAll(target) : (target && target.length ? target : [target]);
    items.forEach((el) => {
      if (!el || !el.style || !vars) return;
      if (Object.prototype.hasOwnProperty.call(vars, "opacity")) el.style.opacity = vars.opacity;
      if (Object.prototype.hasOwnProperty.call(vars, "display")) el.style.display = vars.display;
      if (Object.prototype.hasOwnProperty.call(vars, "filter")) el.style.filter = vars.filter;
      if (Object.prototype.hasOwnProperty.call(vars, "y")) el.style.transform = `translateY(${vars.y}px)`;
      if (Object.prototype.hasOwnProperty.call(vars, "scale")) el.style.transform = `scale(${vars.scale})`;
    });
  }
      ...
      timeline: function (opts) {
        return {
          ...
          progress: function (value) {
            if (value === 1 && opts && typeof opts.onComplete === "function") opts.onComplete();
            return this;
          }
        };
      },
      utils: {
        toArray: function (selector) { return Array.from(document.querySelectorAll(selector)); }
      }
    };
  }
}());
...
/* ─── GOOGLE SHEETS RSVP URL ─────────────────────────────── */

const GOOGLE_SCRIPT_URL = "https://script.google.com/macros/s/AKfycby-0-QPWY_uT1kSPOcjqTlSA02d8ToA4u33RatwI1DJM69iXgbp6fasG2AAxIFqWVCu/exec";
...
if (declineInvite) {
  declineInvite.addEventListener("click", async () => {
    ...
    const declineBtn = declineInvite;
    if (declineBtn) {
...
if (countDays && countHours && countMinutes && countSeconds) {
  const eventDate = new Date("2026-06-28T18:00:00+04:00").getTime();
